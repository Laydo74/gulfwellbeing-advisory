...
type ArticleProps={eyebrow:string;title:string;description:string;sections:{title:string;body:string}[];related:{label:string;to:string}[]};
...
export function JournalArticle({eyebrow,title,description,sections,related}:ArticleProps){
 return <SiteShell><article>
  ...
  <div className="mx-auto grid max-w-6xl gap-16 px-6 py-20 lg:grid-cols-[1fr_.32fr] lg:px-12 lg:py-28">
   <div className="space-y-12">{sections.map(s=><section key={s.title}><h2 className="font-serif text-3xl md:text-4xl">{s.title}</h2><p className="mt-5 text-lg leading-8 text-muted-foreground">{s.body}</p></section>)}
